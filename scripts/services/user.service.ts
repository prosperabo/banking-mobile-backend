...
export async function saveBackofficeProfile(
    ...
    try {
        const createData = {
            ...
            are_account_resources_of_user: backofficeData.are_account_resources_of_user,
            ...
            identification_document_back: backofficeData.identification_document_back,
            identification_document_back_url: backofficeData.identification_document_back_url,
            identification_document_front: backofficeData.identification_document_front,
            identification_document_front_url: backofficeData.identification_document_front_url,
            identification_document_type: backofficeData.identification_document_type,
        ...
        await backofficeRepository.upsertProfile(userId, createData, updateData);
...
export async function saveAuthState(
    ...
    try {
        const createData = {
            ...
            oauthExpirationTimestamp: null as string | null,
            refreshExpirationTimestamp: null as string | null,
            externalCustomerId: backofficeData.id,
            ewalletId: backofficeData.ewallet_id,
            defaultBalanceId: null as number | null,
            lastCardId: null as number | null,
        };
        ...
        await backofficeRepository.upsertAuthState(userId, createData, updateData);
...
export async function hasExistingProfile(userId: number): Promise<boolean> {
    try {
        const existingProfile = await backofficeRepository.findProfileByUserId(userId);
...
export async function getUsersForMigration(
    ...
    try {
        if (targetUserId) {
            const user = await backofficeRepository.findUserById(targetUserId);
        ...
        return await backofficeRepository.findAllUsers();
    } catch (error) {
